    ...
    while (true) {
        ...
        if (std::try_lock(lock1, lock2) == -1) {  // -1 means both locks were successful
            if (from.withdraw(amount)) {
                to.deposit(amount);
                std::cout << "Transaction " << amount << " Finished!\n";
            }
            break;
        } else {
            std::cout << "Deadlock detected. Retrying transfer.\n";
            std::this_thread::sleep_for(std::chrono::milliseconds(5000));  // Retry after a short delay
        }
    }
}
...
void safe_transfer(BankAccount &from, BankAccount &to, int amount) {
    ...
    if (from.withdraw(amount)) {
        to.deposit(amount);
        std::cout << "Safe transaction " << amount << " Finished!\n";
    }
}
